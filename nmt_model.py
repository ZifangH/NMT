...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ## transform shape from (src_len, b, h*2) to (b, src_ken, h*2)
        enc_hiddens = enc_hiddens[0].permute(1, 0, 2)

        # 3. Compute `dec_init_state` = (init_decoder_hidden, init_decoder_cell)
        ## init_decoder_hidden
        h_forwards, h_backwards = last_hidden[0], last_hidden[1]
        h_enc = torch.cat([h_forwards, h_backwards], dim=1)
        init_decoder_hidden = self.h_projection(h_enc)
        ## init_decoder_cell
        c_forwards, c_backwards = last_cell[0], last_cell[1]
        c_enc = torch.cat([c_forwards, c_backwards], dim=1)
        init_decoder_cell = self.c_projection(c_enc)
        ## initial state and cell for decoder
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 4. Convert combined_outputs from a list length tgt_len of tensors shape (b,h) to a single tensor shape (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        U_t = torch.cat([a_t, dec_hidden], dim=1) # dec_hidden: (b, h)  ----> (b, 3h)
